...
import { ContentBlockLink, ContentOverlayLink } from '~/Content/ContentLink'
...
export default function richTextOptions(links) {
  ...
  return (
    {
      ...
      [BLOCKS.HEADING_1]: (node, children) => <Heading color="gray.700" as="h2" lineHeight="1.4" fontWeight="800" fontSize="1.5rem" mb='5'>{children}</Heading>,
          ...
          <Box key={node.data.target.sys.id}>
            ...
            { (entry.__typename == "SharepointDocument") &&
              <Box mb="9" mt="10" px="10">
                <ContentOverlayLink type={entry.__typename} content={entry} />
              </Box>
            }

            { ((entry.__typename != "SharepointDocument") && (entry.__typename != "CodeEmbed")) &&
              <Box mb="9" mt="10" px="10">
                <ContentBlockLink type={entry.__typename} content={entry} />
              </Box>
            }
        ...
        return (
          <>
            ...
            { (!asset.contentType.includes('image') && !ReactPlayer.canPlay(asset.url)) &&
              <ActionButton target="_blank" href={asset.url} mb="4">
